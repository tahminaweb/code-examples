<?php global $post, $more, $aux_content_width; $more = 0; // to enable read more tag
    ...
    extract( $post_vars );
?>                  <div class="col-sm-6 col-md-4 article-bx">
                        <article <?php post_class(); ?> >
                            ...
                            <div class="entry-main">
                                ...
                                <div class="entry-info">
                                    ...
                                    <span class="meta-sep meta-author"><?php /*esc_html_e("by", 'phlox-pro'); */?></span>
                                    ...
                                    <?php if (auxin_is_true($show_post_categories)) : ?>
                                        <span class="entry-tax clientName">
                                       <!-- <img src="http://srbmuk.com/lawfirm/wp-content/uploads/2019/03/7tk6VRIE_400x400-300x300.jpg"
                                             alt="" class="clientName-logo">-->
                                        <?php // the_category(' '); we can use this template tag, but customizable way is needed! ?>
                                            <?php $tax_name = 'gc_company';
                                            if ($cat_terms = wp_get_post_terms($post->ID, $tax_name)) {
                                                foreach ($cat_terms as $term) {
                                                    $key = get_term_meta($term->term_id, 'company_logo', true);
                                                    $image_attributes = wp_get_attachment_image_src($attachment_id = $key);
                                                    $alt_text = get_term_meta($key, '_wp_attachment_image_alt', true);
                                                    if ($image_attributes) :
                                                        echo '<img class="clientName-logo" src="' . $image_attributes[0] . '" alt="' . $term->name . '" title="' . $term->name . '"  />';
                                                    endif;
